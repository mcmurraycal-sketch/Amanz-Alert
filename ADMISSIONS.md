...
**2026-06-18** — Official-notice ingestion: built the pipeline to pull *municipal* notices (not just community reports) into the platform, starting with Johannesburg Water. The interesting part is *why it's hard*: JHB Water — the most systematic publisher in the country — posts its daily interruption notices as **scanned photographs**, with the actual content (which suburbs, what times) locked inside the image where no ordinary scraper can read it. So the design became "automate where the data exists, human-curate where it doesn't": a scheduled job fetches each day's notice image, a vision model reads the picture into structured rows, and those land in a new `official_notices` table separate from crowd reports. Built the table, the ingestion function, and the dedupe logic; **deferred switching it on** because the vision step is a metered paid service and activating it means committing to a small recurring bill — a deliberate "don't spend before launch" call. In the interim the same table is populated by hand. The framing that came out of it is the cleanest articulation of the whole product yet: **Amanz' Alert ingests what municipalities publish and crowd-sources what they don't.** *(Write your own honest line about the moment you realised even the country's best-publishing water utility hides its data inside pictures — what that says about how hard SA's water information actually is to access.)*
...
- **Automate where the data exists, human-curate where it doesn't.** Municipal notices and community reports are two different data problems. Where a municipality publishes machine-readable notices, ingest them automatically; where it publishes scanned images or nothing, fall back to human curation and crowd reports. Johannesburg Water — the most systematic publisher in SA — still posts its daily notices as scanned JPGs, so reading them requires optical character recognition (a vision model), not a scraper. The `official_notices` table is kept deliberately separate from crowd `reports`, because an authoritative planned-maintenance notice and an anonymous "my water's out" report carry different weight and should never be silently merged. (The vision-based ingestion is built but left switched off pre-launch — it's a metered cost, and standing it up before the product has users would be spending for its own sake.)
...
- [~] **Official-notice ingestion (JHB Water)** — pipeline *built* (table + vision-OCR function + dedupe), **switched off pending billing**. Interim: log official notices by hand into the `official_notices` table. Activate the automation post-launch once there are users to justify the small metered cost. Doubles as an essay line: *"automate where the data exists, human-curate where it doesn't."*
...
- *"Amanz' Alert ingests what municipalities publish and crowd-sources what they don't — because even Johannesburg Water, the country's most systematic publisher, still hides its daily notices inside scanned images that no ordinary scraper can read."*
